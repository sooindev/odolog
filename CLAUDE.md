...
9-1. **차량의 숫자 PK 는 서버 밖으로 내보내지 않는다** (2026-09-26). URL·API 에는 12자 무작위
    ...
    **정비·주유 기록 id 는 아직 숫자다** — 주소창에 안 뜨고 API 경로에만 보여서 범위에서 뺐다.
...
**테스트는 대상과 같은 경로를 그대로 따라간다.** 총 252개.
...
대상과 맞추는 것과 같다. `npm run test` 로 돌리고 **총 44개**다.
...
- [ ] **B-23** 상세 진입 → eyebrow 가 **번호판**, 뒤로가기 있음. 머리말이 사이드바 밖 전체 폭인지
      ...
      → ⚠️ 운영 DB 는 **앱을 띄우기 전에** README 의 "차량 공개 id" SQL 을 먼저 실행해야 한다.
        안 하면 기존 차량 링크가 전부 `/vehicles/` 로 깨진다
...
- [ ] 백엔드 테스트 전체 통과 — `./gradlew test` (252개)
