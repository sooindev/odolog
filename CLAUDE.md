...
**테스트는 대상과 같은 경로를 그대로 따라간다.** 총 125개.
...
→ 눈 확인은 **Phase 6 의 6-B(1회차)** 로 합쳤다. 해당 항목은 B-10 ~ B-12, B-82 ~ B-83.

---

## Phase 4 — 차량 관리 화면 (완료)

→ 눈 확인은 **Phase 6 의 6-B(1회차)** 로 합쳤다. 해당 항목은 B-13 ~ B-31, B-86.
...
재설계 → 모서리 각지게 → 모션 재조정 → 모바일 최적화까지 네 번 연속으로 화면을 크게 고쳤는데,
전부 `tsc -b` / `oxlint` / `vite build` 통과만으로 끝냈다. **실제 화면은 아직 아무도 본 적이
없다.** 여기에 코드를 더 쌓으면 깨진 게 나왔을 때 어느 작업에서 깨졌는지 못 찾는다.
...
- [ ] **A-6** `odolog` 스키마에는 사용자가 없다. **회원가입부터 시작**한다
...
- [ ] **B-32** 다음 정비 카드 — 종류 5개(엔진오일·타이어·브레이크 패드·배터리·기타)가 전부
      "기록 없음". 3열(종류 / 마지막 정비 / 다음 정비)인지
...
- [ ] **B-35** 날짜 입력칸의 기본값이 **오늘**인지. UTC 가 아니라 로컬 기준(`todayString()`)이라
      **오전 9시 이전에도 어제가 아닌지**
      → ⚠️ 이 칸은 **기기에 따라 컨트롤이 갈린다.** 마우스·키보드면 네이티브 date 입력,
        ...
        기준이라 네이티브 쪽을 본다. 휠은 **6-C 의 C-14~C-19** 에서 따로 확인한다
...
- [ ] **B-45** 기록이 0건일 때 연비 카드가 "2건 이상 쌓이면 계산합니다" 안내를 띄우는지
      (0.00 km/L 이 찍히면 안 된다)
...
- [ ] **B-50** 첫 기록 등록 → 목록에 **"연비 —"** 로 뜨는지. **0.00 이면 잘못된 것이다**
      (직전 기록이 없으면 null 이어야 한다)
...
- [ ] **B-53** 두 번째 기록 등록(주행거리를 500km 올리고 25L) → 연비가 **20.00 km/L** 인지.
      손으로 나눠서 맞춰 볼 것
- [ ] **B-54** 연비 카드의 **평균 연비 히어로 숫자**가 뜨는지. 통계 4칸(기록·주행·주유량·총 유류비)
- [ ] **B-55** 히어로 숫자에 `tabular-nums` 가 **안 붙어 있는지**(맞출 상대가 없는 큰 숫자)
- [ ] **B-56** 목록이 **주행거리 내림차순**인지(최신이 위). 날짜순이 아니다 — 정렬이 곧 연비
      계산의 전제라 서버가 고정한다
- [ ] **B-57** 기록을 11건 이상 만들어 **페이지네이션**. ⚠️ **2페이지 첫 행에도 연비가 나오는지** —
      그 행의 짝은 1페이지에 있어서 서버가 따로 한 건 더 조회한다. 여기가 비어 있으면 버그다
- [ ] **B-58** 중간 기록의 **주행거리를 수정** → 그 기록과 **바로 뒤 기록의 연비가 둘 다** 바뀌는지
      (계산 값을 저장하지 않고 읽을 때 계산하는 이유가 이것이다)
- [ ] **B-59** 기록 삭제 → 확인 문구에 "연비가 다시 계산됩니다"가 있는지. 삭제 후 연비가 바뀌는지
- [ ] **B-60** 삭제 중 **그 행만** 잠기는지(목록 전체가 아니라)

#### 6-B-6. 홈 통계와 차트

**여기가 눈으로 처음 보는 화면이다** — 통계·차트는 만든 뒤 한 번도 못 봤다.

- [ ] **B-61** `/` → 통계 화면(세 번째 얼굴). 타일 4개: 차량 수 · 총 주행거리 · 정비 건수 · 총 비용
- [ ] **B-62** 타일 값에 `tabular-nums` 가 **안 붙어 있는지**(큰 숫자 하나는 맞출 상대가 없다)
- [ ] **B-63** 타일 사이 `gap-px` 격자의 선이 1px 인지. 칸 배경이 새지 않는지
- [ ] **B-64** 월별 비용 차트 — **12칸이 항상 다 있는지**. 기록 없는 달도 빈 칸으로 남아야 한다
      (기록 있는 달만 모으면 띄엄띄엄 정비한 것이 꾸준히 한 것처럼 보인다)
- [ ] **B-65** 막대 **네 모서리가 전부 각진지**, 두께가 24px 이하인지
- [ ] **B-66** 세로축 눈금이 `1,873` 같은 수가 아니라 **1·2·5 × 10ⁿ** 으로 끝나는지(`niceMax()`)
- [ ] **B-67** 눈금선이 **점선이 아니라 1px 실선**인지(점선은 "예측·임계선"으로 읽힌다)
- [ ] **B-68** **가장 높은 막대 하나에만** 값이 적혀 있는지(전부 적으면 축이 무의미해진다)
- [ ] **B-69** 막대에 마우스 → 말풍선. 판정 영역이 막대(24px)가 아니라 **칸 전체**인지
- [ ] **B-70** `<details>` 표를 펼쳐 말풍선과 **같은 값**이 나오는지
- [ ] **B-71** 종류별 가로 막대 — **전부 같은 색**인지(클수록 진하게가 아니다). **범례가 없는지**
- [ ] **B-72** 막대가 자라는 동안 모양이 찌그러지지 않는지(`clip-path`, 칸마다 45ms)

#### 6-B-7. 프로필 · 비밀번호 · 탈퇴 · 계정 격리

- [ ] **B-73** 헤더 닉네임 클릭 → `/me`. eyebrow `ACCOUNT`, **`Section` 4개**
      (계정 / 비밀번호 / 화면 / 회원 탈퇴)
- [ ] **B-74** 닉네임만 변경 → PATCH 바디에 `nickname` 하나만. 헤더 표시도 같이 바뀌는지
- [ ] **B-75** '화면' 구역의 설명이 현재 테마를 맞게 말하는지(`system` 이면 "지금은 ○○입니다")
- [ ] **B-76** '비밀번호' 구역 — 새 비밀번호 두 칸이 **서로 다르게** 입력되면 폼 안에서 막히는지
      (서버로 보내지 않는다. 확인란은 오타 방지 장치일 뿐이다)
- [ ] **B-77** 새 비밀번호를 7자로 → 400. 가입 때와 같은 제한인지
- [ ] **B-78** ⚠️ **현재 비밀번호를 틀리게 입력 → 401 이 폼 안에 뜨는데 로그아웃되지 않는지.**
      작업 중 실제로 냈던 버그다 — 전역 401 핸들러가 돌면 오타 한 번에 `/login` 으로 쫓겨난다
- [ ] **B-79** 변경 성공 → 안내 문구 + **입력칸 3개가 비워지는지** + **로그인이 유지되는지**
- [ ] **B-80** 로그아웃 후 **새 비밀번호로** 로그인되는지. 옛 비밀번호로는 안 되는지
- [ ] **B-81** 비밀번호 관리자를 쓴다면 '현재'와 '새것'을 구분해 채우는지(`autoComplete`)
- [ ] **B-82** 로그아웃 → 헤더가 `로그인` 버튼으로 돌아가는지
- [ ] **B-83** 주소창에 `/vehicles` 직접 입력 → **`/login` 으로 튕기는지**.
      로그인하면 **원래 가려던 `/vehicles` 로 복귀**하는지
- [ ] **B-84** **두 번째 계정**으로 가입 → 차량 목록이 비어 있는지(**남의 차량이 안 보인다**)
- [ ] **B-85** 두 번째 계정에서 첫 계정 차량의 상세 주소(`/vehicles/1` 등)를 직접 입력 →
      404 와 403 을 **구분해서 보여주지 않는지**(남의 차량 존재 자체를 알리지 않는다)
- [ ] **B-86** 첫 계정 복귀 → 차량 삭제(`window.confirm` 에 "정비 이력도 함께 삭제" 명시) →
...
- [ ] **B-87** '회원 탈퇴' 구역이 **접힌 채로** 시작하는지. 버튼이 빨갛게 **채워져 있지 않은지**
- [ ] **B-88** 탈퇴에서 비밀번호를 **틀리게** → 401 이 폼 안에 뜨고 **로그인이 유지되는지**
- [ ] **B-89** 탈퇴 성공 → `/` 로 이동하고 헤더가 로그아웃 상태인지.
      **뒤로가기로 방금 화면에 돌아가지지 않는지**(`replace: true`)
- [ ] **B-90** 탈퇴한 계정으로 로그인 시도 → 401
- [ ] **B-91** 탈퇴 후 DB 에 그 사용자의 차량·정비 이력·주유 기록이 **하나도 안 남았는지**
- [ ] **B-92** 없는 주소(`/asdf`) → `/` 로 가는지(`/vehicles` 로 보내면 비로그인 사용자가 또 튕긴다)
- [ ] **B-93** 화면을 옮겨 다니며 **페이지 전환이 0.18s 페이드만**인지.
...
- [ ] **C-9 · 1024px** 차량 상세가 2단인지. 폼 화면이 `Section` 2단인지
- [ ] **C-10 · 1024px** 긴 메모·긴 닉네임을 넣어도 **격자가 넘치지 않는지**(`min-w-0`)
- [ ] **C-11 · 1440px** 제목이 52px 까지 커지는지. 글이 담긴 열이 **700px 를 안 넘는지**
- [ ] **C-12 · 1440px** 본문이 76rem 에서 멈추고 가운데 정렬인지
- [ ] **C-13** 폭을 **천천히 끌어서** 줄여 본다 — 특정 폭에서만 깨지는 자리가 있는지
...
- [ ] **C-14** 정비 폼·주유 폼의 날짜 칸이 **캘린더가 아니라** `2026. 9. 17.  변경` 모양인지.
      눌렀을 때 년/월/일 세 칸이 펼쳐지는지
- [ ] **C-15** ⚠️ **칸을 끝까지 세게 굴려도 페이지가 안 튀는지.** 남은 관성이 바깥으로 새면
      (scroll chaining) 화면이 통째로 스크롤된다. `overscroll-contain` 이 막고 있는 지점이다
- [ ] **C-16** ⚠️ **폼 맨 아래 날짜 칸에서 눌렀을 때 휠이 화면 안으로 따라 들어오는지.**
      이미 잘 보이는 자리에서는 **화면이 움직이지 않아야** 한다(`block: 'nearest'`)
- [ ] **C-17** 손가락을 떼면 칸이 **딱 맞춰 멈추는지**(scroll-snap). 반쯤 걸치지 않는지
- [ ] **C-18** 1월 31일로 맞춘 뒤 **월을 2월로** 굴리기 → 일이 **28(윤년이면 29)로 끌려오는지**.
      31일 자리에 멈춘 채 값만 바뀌면 위치와 값이 어긋난 것이다
- [ ] **C-19** 굴리는 **도중에는 값이 안 바뀌고** 멈춘 뒤에 바뀌는지.
      `변경` 을 다시 눌러(`완료`) 접으면 고른 날짜가 위에 그대로 있는지
- [ ] **C-20** 값이 폼 제출에 제대로 실리는지 — 등록 후 목록의 날짜가 고른 날인지
...
- [ ] **D-10** 브라우저 탭의 **파비콘이 각진지**(오늘 고쳤다. 캐시가 있으면 하드 리로드 `⌘⇧R`)
...
**Phase 6 의 1회차(6-B)를 순서대로 따라가면 이 10개가 전부 덮인다.** 오른쪽이 그 항목 번호다 —
따로 한 번 더 돌 필요가 없다.

- [ ] 회원가입 → 로그아웃 → 로그인 — B-10, B-82
...
- [ ] 주유 기록 등록/수정/삭제 — B-46, B-58, B-59
- [ ] 연비가 계산되고, 첫 기록은 빈 값(`—`)으로 표시됨 — B-50, B-53
- [ ] 페이지가 넘어가도 연비가 끊기지 않음 — B-57
- [ ] 주유 기록이 차량 주행거리를 따라 올림 — B-51
- [ ] 비밀번호 변경, 현재 비밀번호를 틀려도 로그아웃되지 않음 — B-78, B-79
- [ ] 회원 탈퇴 후 그 계정의 데이터가 남지 않음 — B-90, B-91
- [ ] 차량 삭제 시 정비 이력·주유 기록도 함께 사라짐 — B-86
- [ ] 로그인 안 한 상태로 `/vehicles` 직접 접근 시 로그인 페이지로 이동 — B-83
- [ ] 다른 계정으로 로그인했을 때 남의 차량이 안 보임 — B-84, B-85
- [ ] 백엔드 테스트 전체 통과 — `./gradlew test` (125개)
