...
**테스트는 대상과 같은 경로를 그대로 따라간다.** 총 140개.
...
- [x] 눈 확인 체크리스트·문서 최신화 — 코드를 못 따라간 곳 (2026-09-17)
      ...
      → **화면과 어긋난 것 10건을 고쳤다.** 전부 "코드는 고쳤는데 체크리스트가 옛 화면을
        ...
        · **B-48 / B-53** 연비 없을 때의 문구 — `연비 —` 는 9/17 에 이유를 말하는 두 문구로
          갈라졌다(`기준 기록` / `연비 기준점`).
        · **B-72 / B-76 / B-82 / B-83** 홈 — `Records`·`Cost` 가 주유를 포함하고 월별 차트가
          `유지비` 가 된 것(9/16), 표의 `정비/주유/합계` 열.
        · **B-101** 차량 삭제 confirm — 주유 기록이 빠진 옛 문구를 그대로 적고 있었다.
...
→ 눈 확인은 **Phase 6 의 6-B(1회차)** 로 합쳤다. 해당 항목은 B-10 ~ B-12, B-97 ~ B-98.

---

## Phase 4 — 차량 관리 화면 (완료)

→ 눈 확인은 **Phase 6 의 6-B(1회차)** 로 합쳤다. 해당 항목은 B-13 ~ B-33, B-101.
...
- [ ] **A-6** 시작 데이터를 정한다. **2026-09-17 기준 운영 스키마는 비어 있지 않다** —
      ...
      1회차는 **회원가입부터 데이터가 쌓이는 순서**로 짜여 있고, 특히 B-99(남의 차량이 안 보임) ·
      B-101(차량 삭제 시 동반 삭제를 `COUNT(*)` 로 확인) · B-106(탈퇴 후 0건)은 **빈 상태여야
...
- [ ] **B-64** 구간을 **고르게 3개 이상** 쌓은 뒤 중간 주유 하나를 삭제 → 연비 카드에
...
- [ ] **B-65** 카드 오른쪽 위 `연비 초기화` → confirm **"지금까지의 기록을 연비 계산에서
      빼고 다시 셉니다. 계속할까요?"**
- [ ] **B-66** 확인 → 평균 연비 히어로 숫자가 사라지고 **"연비를 초기화했습니다. 다음 주유
      기록부터 다시 계산합니다."** 로 바뀌는지
- [ ] **B-67** ⚠️ **같은 순간 목록도 갱신되는지** — 기준점이 된 행이 `연비 기준점 · 다음
      주유부터 계산` 으로 바뀌어야 한다. 카드만 바뀌고 목록이 옛 연비를 들고 있으면
      `fuelListVersion` 이 안 도는 것이다(작업 중 실제로 빠뜨렸던 자리)
- [ ] **B-68** ⚠️ **통계 4칸(기록·주행·주유량·총 유류비)은 그대로인지.** 초기화는 연비만
      다시 세는 것이지 **지출을 없던 일로 만드는 게 아니다.** 여기가 줄어들면 잘못된 것이다
- [ ] **B-69** 주유를 한 건 더 등록 → 평균이 다시 나오고, 그 아래 **"연비 초기화 이후 구간만
      계산한 값입니다."** 가 붙는지. `초기화 해제` 를 누르면 전체 평균으로 돌아오는지
- [ ] **B-70** 기록 삭제 → 확인 문구에 "연비가 다시 계산됩니다"가 있는지. 삭제 후 연비가 바뀌는지
- [ ] **B-71** 삭제 중 **그 행만** 잠기는지(목록 전체가 아니라)

#### 6-B-6. 홈 통계와 차트

**여기가 눈으로 처음 보는 화면이다** — 통계·차트는 만든 뒤 한 번도 못 봤다.

- [ ] **B-72** `/` → 통계 화면(세 번째 얼굴). eyebrow 가 `Overview`(0대일 때의 `Garage` 가 아니다),
      타일 4개는 `Vehicles` · `Distance` · `Records` · `Cost`.
      ⚠️ **`Records` 는 정비 + 주유 건수 합계**이고 **`Cost` 는 정비비 + 유류비**다 —
      정비만 세고 있으면 9/16 에 고친 것이 되돌아간 것이다
- [ ] **B-73** `Cost` 타일 아래에 **`정비 N · 주유 N`** 구성 한 줄이 붙는지.
      합계만 주면 어느 쪽이 큰지 알 수 없어서 둔 줄이다
      → 여기서 **"일부 기록만 합산됨" 같은 단서가 보이면 안 된다.** 요약 API 가 서버에서
        전부 더하므로 상한이 없어졌고, 화면이 변명하던 자리는 사라졌다
- [ ] **B-74** 타일 값에 `tabular-nums` 가 **안 붙어 있는지**(큰 숫자 하나는 맞출 상대가 없다)
- [ ] **B-75** 타일 사이 `gap-px` 격자의 선이 1px 인지. 칸 배경이 새지 않는지
- [ ] **B-76** 차트 제목이 **`지난 12개월 유지비`**(정비 + 주유)인지. 정비비만 세는 옛 제목이면
      타일의 `Cost` 와 기준이 달라져 같은 화면에 두 가지 총액이 놓인다.
      **12칸이 항상 다 있는지** — 기록 없는 달도 빈 칸으로 남아야 한다
      (기록 있는 달만 모으면 띄엄띄엄 정비한 것이 꾸준히 한 것처럼 보인다)
- [ ] **B-77** 막대 **네 모서리가 전부 각진지**, 두께가 24px 이하인지
- [ ] **B-78** 세로축 눈금이 `1,873` 같은 수가 아니라 **1·2·5 × 10ⁿ** 으로 끝나는지(`niceMax()`)
- [ ] **B-79** 눈금선이 **점선이 아니라 1px 실선**인지(점선은 "예측·임계선"으로 읽힌다)
- [ ] **B-80** **가장 높은 막대 하나에만** 값이 적혀 있는지(전부 적으면 축이 무의미해진다)
- [ ] **B-81** 막대에 마우스 → 말풍선. 판정 영역이 막대(24px)가 아니라 **칸 전체**인지
- [ ] **B-82** `<details>` 표를 펼쳐 말풍선과 **같은 값**이 나오는지.
      열이 **`정비 / 주유 / 합계`** 로 나뉘어 있는지 — 말풍선에만 구성이 있으면
      **키보드·스크린리더 사용자에게는 없는 값**이다
- [ ] **B-83** 종류별 가로 막대 — **전부 같은 색**인지(클수록 진하게가 아니다). **범례가 없는지**.
      아래에 **"유류비는 포함하지 않습니다"** 가 적혀 있는지 — 주유는 정비 종류가 아니라
      여기 들어갈 자리가 없는데, 옆 차트가 '유지비'가 되면서 헷갈리기 쉬워졌다
- [ ] **B-84** 막대가 자라는 동안 모양이 찌그러지지 않는지(`clip-path`, 칸마다 45ms)
- [ ] **B-85** `최근 활동` 카드에 **정비와 주유가 섞여** 나오는지(`최근 정비` 가 아니다).
      각 줄이 어느 쪽인지 구분되는지
- [ ] **B-86** **같은 날짜의 정비와 주유** → 정비가 위인지. ⚠️ 여기서 id 로 비교하면 안 된다 —
      테이블이 달라 주유 3번이 정비 3번보다 나중이라는 보장이 없다
- [ ] **B-87** `차량별` 카드에 **평균 연비**(`N.Nkm/L`)가 붙는지. 주유 2건 미만인 차량은
      그 자리를 **아예 비우는지**(`연비 —` 를 붙이면 없는 값이 자리를 차지한다)

#### 6-B-7. 프로필 · 비밀번호 · 탈퇴 · 계정 격리

- [ ] **B-88** 헤더 닉네임 클릭 → `/me`. eyebrow `ACCOUNT`, **`Section` 4개**
      (계정 / 비밀번호 / 화면 / 회원 탈퇴)
- [ ] **B-89** 닉네임만 변경 → PATCH 바디에 `nickname` 하나만. 헤더 표시도 같이 바뀌는지
- [ ] **B-90** '화면' 구역의 설명이 현재 테마를 맞게 말하는지(`system` 이면 "지금은 ○○입니다")
- [ ] **B-91** '비밀번호' 구역 — 새 비밀번호 두 칸이 **서로 다르게** 입력되면 폼 안에서 막히는지
      (서버로 보내지 않는다. 확인란은 오타 방지 장치일 뿐이다)
- [ ] **B-92** 새 비밀번호를 7자로 → 400. 가입 때와 같은 제한인지
- [ ] **B-93** ⚠️ **현재 비밀번호를 틀리게 입력 → 401 이 폼 안에 뜨는데 로그아웃되지 않는지.**
      작업 중 실제로 냈던 버그다 — 전역 401 핸들러가 돌면 오타 한 번에 `/login` 으로 쫓겨난다
- [ ] **B-94** 변경 성공 → 안내 문구 + **입력칸 3개가 비워지는지** + **로그인이 유지되는지**
- [ ] **B-95** 로그아웃 후 **새 비밀번호로** 로그인되는지. 옛 비밀번호로는 안 되는지
- [ ] **B-96** 비밀번호 관리자를 쓴다면 '현재'와 '새것'을 구분해 채우는지(`autoComplete`)
- [ ] **B-97** 로그아웃 → 헤더가 `로그인` 버튼으로 돌아가는지
- [ ] **B-98** 주소창에 `/vehicles` 직접 입력 → **`/login` 으로 튕기는지**.
      로그인하면 **원래 가려던 `/vehicles` 로 복귀**하는지
- [ ] **B-99** **두 번째 계정**으로 가입 → 차량 목록이 비어 있는지(**남의 차량이 안 보인다**)
- [ ] **B-100** 두 번째 계정에서 첫 계정 차량의 상세 주소(`/vehicles/1` 등)를 직접 입력 →
      404 와 403 을 **구분해서 보여주지 않는지**(남의 차량 존재 자체를 알리지 않는다)
- [ ] **B-101** 첫 계정 복귀 → 차량 삭제. ⚠️ confirm 문구가 **"이 차량과 정비 이력, 주유 기록이
...
- [ ] **B-102** '회원 탈퇴' 구역이 **접힌 채로** 시작하는지. 버튼이 빨갛게 **채워져 있지 않은지**
- [ ] **B-103** 탈퇴에서 비밀번호를 **틀리게** → 401 이 폼 안에 뜨고 **로그인이 유지되는지**
- [ ] **B-104** 탈퇴 성공 → `/` 로 이동하고 헤더가 로그아웃 상태인지.
      **뒤로가기로 방금 화면에 돌아가지지 않는지**(`replace: true`)
- [ ] **B-105** 탈퇴한 계정으로 로그인 시도 → 401
- [ ] **B-106** 탈퇴 후 DB 에 그 사용자의 차량·정비 이력·주유 기록이 **하나도 안 남았는지**
- [ ] **B-107** 없는 주소(`/asdf`) → `/` 로 가는지(`/vehicles` 로 보내면 비로그인 사용자가 또 튕긴다)
- [ ] **B-108** 화면을 옮겨 다니며 **페이지 전환이 0.18s 페이드만**인지.
...
**Phase 6 의 1회차(6-B) 108개를 순서대로 따라가면 아래가 전부 덮인다.** 오른쪽이 그 항목
번호다 — 따로 한 번 더 돌 필요가 없다.

- [ ] 회원가입 → 로그아웃 → 로그인 — B-10, B-97
...
- [ ] 주유 기록 등록/수정/삭제 — B-49, B-62, B-70
- [ ] 연비가 계산되고, 첫 기록은 `기준 기록 · 다음 주유부터 계산` 으로 이유까지 말함 — B-53, B-57
- [ ] 페이지가 넘어가도 연비가 끊기지 않음 — B-61
- [ ] 주유 기록이 차량 주행거리를 따라 올림 — B-54
- [ ] 연비 초기화 — 기록·지출은 그대로 두고 연비만 다시 셈 — B-65, B-68, B-69
- [ ] 빠진 주유 기록을 평소 구간과 견줘 알려 줌 — B-64
- [ ] 비밀번호 변경, 현재 비밀번호를 틀려도 로그아웃되지 않음 — B-93, B-94
- [ ] 회원 탈퇴 후 그 계정의 데이터가 남지 않음 — B-105, B-106
- [ ] 차량 삭제 시 정비 이력·주유 기록도 함께 사라짐 — B-101
- [ ] 로그인 안 한 상태로 `/vehicles` 직접 접근 시 로그인 페이지로 이동 — B-98
- [ ] 다른 계정으로 로그인했을 때 남의 차량이 안 보임 — B-99, B-100
- [ ] 백엔드 테스트 전체 통과 — `./gradlew test` (140개)
