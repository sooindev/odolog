...
### 프론트엔드 — `frontend/`
    ...
    └── src/
        ...
        │   ├── LandingPage.tsx       '/' 소개 화면. API·상태 없이 shared/ui 조립만 하는
        │   │                         화면이라 features/ 가 아니라 여기 있다
...
- [x] 소개 화면(랜딩) 신설 — `/` (2026-09-11)
      ...
      → 로고가 가리키는 곳을 로그인 여부로 나눴다(`/` vs `/vehicles`). 로그인한 사람에게 홈은
        소개 화면이 아니라 자기 차량 목록이다.
...
- [ ] 백엔드 400 검증 응답과 폼 필드 연결
      → 현재 `ErrorResponse`는 `message` 하나뿐이라 **어느 필드가 틀렸는지 모른다.**
        ...
        (백로그 항목으로 이관).
- [ ] 브라우저에서 재설계 결과 확인 — **8개 화면** 전부, **375px / 1024px / 1440px 세 폭**
...
(아래 2026-09-11 항목). **한때 "차량 목록을 그리드로 바꾸지 않는다"고 적어 두었으나 뒤집혔다** —
