...
14. **주석은 한 줄 명사구로 쓴다** (2026-09-18 부터). 종결어미(`~한다` / `~이다`)를 붙이지 않고
    ...
    **2026-09-27 에 프로젝트 전체 주석(약 1,100덩어리)을 이 기준으로 다시 썼다.** 함께 정한 것:
    경위("전에는 ~였다")·날짜·줄표(—) 연쇄·굵게·⚠️ 같은 기호는 주석에 쓰지 않는다 —
    경위는 HISTORY.md, 긴 이유는 이 문서의 몫이다. 예시 `// 이력 먼저, 차량 나중 — FK 제약` 처럼
    줄표가 꼭 필요하면 한 번만. 새 주석도 같은 모양으로 쓴다.
