...
→ 눈 확인은 **Phase 6 의 6-B(1회차)** 로 합쳤다. 해당 항목은 B-10 ~ B-12, B-53 ~ B-54.

---

## Phase 4 — 차량 관리 화면 (완료)

→ 눈 확인은 **Phase 6 의 6-B(1회차)** 로 합쳤다. 해당 항목은 B-13 ~ B-25, B-57.

---

## Phase 5 — 정비 이력 관리 화면 (완료)

→ 눈 확인은 **Phase 6 의 6-B(1회차)** 로 합쳤다. 해당 항목은 B-26 ~ B-37.
...
- [ ] **B-26** 다음 정비 카드 — 종류 5개(엔진오일·타이어·브레이크 패드·배터리·기타)가 전부
      "기록 없음". 3열(종류 / 마지막 정비 / 다음 정비)인지
- [ ] **B-27** 정비 폼 열기 → **자리를 밀어내며 0.36s 펼쳐지고**, 안쪽 글자는 0.12s 늦게 들어오는지
      (동시에 나타나면 글자가 찌그러지며 늘어나 보인다)
- [ ] **B-28** 폼 닫기 → **연출 없이 즉시** 닫히는지(이미 사용자가 결정한 일이다)
- [ ] **B-29** 날짜 입력칸의 기본값이 **오늘**인지. UTC 가 아니라 로컬 기준(`todayString()`)이라
      **오전 9시 이전에도 어제가 아닌지**
- [ ] **B-30** 엔진오일 이력 등록 → 목록에 표시
- [ ] **B-31** 다음 정비 카드가 갱신되는지. **주행거리 기준과 날짜 기준 둘 다** 나오는지
      (`NextServiceCard` 가 `key` 로 재생성되므로 "불러오는 중…"이 잠깐 보이는 건 정상)
- [ ] **B-32** `기타(OTHER)` 로 등록 → 다음 정비가 **계산되지 않는지**(주기가 둘 다 null)
- [ ] **B-33** **같은 날짜**로 두 건 등록 → 목록 순서가 뒤집히지 않는지
      (동점 기준 `id DESC`. 이게 없으면 새로고침마다 순서가 바뀐다)
- [ ] **B-34** 이력 수정에서 **비용만** 바꾸기 → Network 의 PATCH 요청 바디에
      **`cost` 하나만** 들어 있는지(바뀐 필드만 보낸다)
- [ ] **B-35** 이력 삭제 → 204. 삭제되는 **그 행만** 잠기고 목록 전체가 잠기지 않는지
      (`deletingId` 로 잠근다. boolean 하나면 어느 줄을 지우는 중인지 안 보인다)
- [ ] **B-36** 이력을 11건 이상 등록 → 페이지네이션. 2페이지에서 한 건 삭제 시
      요청이 **두 번 나가지 않는지**(Network 탭에서 GET 개수 확인)
- [ ] **B-37** 차량 상세가 `lg` 에서 2단인지. **왼쪽 사이드바가 스크롤에 붙어 따라오는지**
...
- [ ] **B-38** `/` → 통계 화면(세 번째 얼굴). 타일 4개: 차량 수 · 총 주행거리 · 정비 건수 · 총 비용
- [ ] **B-39** 타일 값에 `tabular-nums` 가 **안 붙어 있는지**(큰 숫자 하나는 맞출 상대가 없다)
- [ ] **B-40** 타일 사이 `gap-px` 격자의 선이 1px 인지. 칸 배경이 새지 않는지
- [ ] **B-41** 월별 비용 차트 — **12칸이 항상 다 있는지**. 기록 없는 달도 빈 칸으로 남아야 한다
      (기록 있는 달만 모으면 띄엄띄엄 정비한 것이 꾸준히 한 것처럼 보인다)
- [ ] **B-42** 막대 **네 모서리가 전부 각진지**, 두께가 24px 이하인지
- [ ] **B-43** 세로축 눈금이 `1,873` 같은 수가 아니라 **1·2·5 × 10ⁿ** 으로 끝나는지(`niceMax()`)
- [ ] **B-44** 눈금선이 **점선이 아니라 1px 실선**인지(점선은 "예측·임계선"으로 읽힌다)
- [ ] **B-45** **가장 높은 막대 하나에만** 값이 적혀 있는지(전부 적으면 축이 무의미해진다)
- [ ] **B-46** 막대에 마우스 → 말풍선. 판정 영역이 막대(24px)가 아니라 **칸 전체**인지
- [ ] **B-47** `<details>` 표를 펼쳐 말풍선과 **같은 값**이 나오는지
- [ ] **B-48** 종류별 가로 막대 — **전부 같은 색**인지(클수록 진하게가 아니다). **범례가 없는지**
- [ ] **B-49** 막대가 자라는 동안 모양이 찌그러지지 않는지(`clip-path`, 칸마다 45ms)

#### 6-B-6. 프로필 · 로그아웃 · 계정 격리

- [ ] **B-50** 헤더 닉네임 클릭 → `/me`. eyebrow `ACCOUNT`, `Section` 2개(계정 / 화면)
- [ ] **B-51** 닉네임만 변경 → PATCH 바디에 `nickname` 하나만. 헤더 표시도 같이 바뀌는지
- [ ] **B-52** '화면' 구역의 설명이 현재 테마를 맞게 말하는지(`system` 이면 "지금은 ○○입니다")
- [ ] **B-53** 로그아웃 → 헤더가 `로그인` 버튼으로 돌아가는지
- [ ] **B-54** 주소창에 `/vehicles` 직접 입력 → **`/login` 으로 튕기는지**.
      로그인하면 **원래 가려던 `/vehicles` 로 복귀**하는지
- [ ] **B-55** **두 번째 계정**으로 가입 → 차량 목록이 비어 있는지(**남의 차량이 안 보인다**)
- [ ] **B-56** 두 번째 계정에서 첫 계정 차량의 상세 주소(`/vehicles/1` 등)를 직접 입력 →
      404 와 403 을 **구분해서 보여주지 않는지**(남의 차량 존재 자체를 알리지 않는다)
- [ ] **B-57** 첫 계정 복귀 → 차량 삭제(`window.confirm` 에 "정비 이력도 함께 삭제" 명시) →
      **이력도 같이 사라졌는지 DB 로 확인**:
      `/opt/homebrew/opt/mariadb/bin/mariadb --no-defaults -e "USE odolog; SELECT COUNT(*) FROM maintenance_records;"`
- [ ] **B-58** 없는 주소(`/asdf`) → `/` 로 가는지(`/vehicles` 로 보내면 비로그인 사용자가 또 튕긴다)
- [ ] **B-59** 화면을 옮겨 다니며 **페이지 전환이 0.18s 페이드만**인지.
