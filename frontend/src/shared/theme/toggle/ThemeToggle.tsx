...
/** 칸 하나의 크기(px). 알약을 몇 px 옮길지 계산하는 데 그대로 쓰인다. */
const CELL = 28
...
export function ThemeToggle({ className }: { className?: string }) {
  ...
  return (
    ...
    <div
      ...
      aria-label="화면 모드"
      className={cn(
        'relative flex items-center rounded-full border border-border bg-sunken p-[3px]',
        className,
      )}
    >
      {/*
        움직이는 알약. 버튼마다 배경을 켜고 끄는 대신 하나짜리 요소를 옮긴다.
        배경을 켜고 끄면 "꺼짐 → 켜짐" 두 애니메이션이 따로 돌아 툭 끊기는데,
        하나를 옮기면 선택이 한 덩어리로 이어져 움직인다.
      */}
      <span
        aria-hidden="true"
        className="absolute top-[3px] left-[3px] rounded-full border border-border bg-fill transition-transform duration-300 ease-apple"
      ...
      {OPTIONS.map(({ value, label, Icon }) => (
        <button
          ...
          className={cn(
            // z-10: 위의 알약보다 위에 있어야 클릭이 버튼에 닿는다.
            'relative z-10 grid place-items-center rounded-full transition-colors duration-200 ease-apple focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background',
