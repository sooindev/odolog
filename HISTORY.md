...
- [x] 프로젝트 전체 주석을 명사형 한 줄로 (2026-09-27)
      → 규칙 14(2026-09-18)를 정해 놓고 그 뒤에 쓴 주석이 오히려 길어졌다 — 경위·날짜·대안 비교가
        코드 옆에 쌓여 한 덩어리가 열 줄을 넘는 곳이 많았다. 같은 설명이 CLAUDE.md·HISTORY.md 에도
        있어서 한쪽만 고치면 코드 옆 설명부터 낡는다(규칙 14 가 경고한 그대로).
      → 백엔드 본 코드·테스트, 프론트, CSS·yml·html·CI 까지 약 1,100덩어리. 주석 줄 2,200 → 1,200 안팎.
      → **코드는 한 글자도 안 바꿨다.** 파일을 통째로 다시 쓰지 않고, 주석 덩어리만 줄 범위로 찾아
        바꾸는 도구를 만들었다. 바꾸기 전 원래 내용의 해시를 대조해 어긋나면 멈추고(실제로 한 번 멈췄다 —
        줄 범위를 잘못 적은 것), 끝나면 "주석을 뺀 코드" 가 git HEAD 와 같은지 176개 파일 전부 비교했다.
        그 뒤 백엔드 264·프론트 45 테스트, tsc·oxlint·vite build 통과.
      → 남긴 것: `/// <reference types="vite/client" />`(주석 모양의 TypeScript 지시문), 이미 짧은
        명사형이던 67덩어리, 화면에 실제로 찍히는 기호를 적은 `— 원` 같은 표기.

- [x] 파일 단위 점검에서 나온 9건 (2026-09-27)
      → 백엔드·프론트 전 파일을 한 장씩 읽고, 의심 가는 것은 테스트 서버에 실제 요청으로 확인했다.
      → **최근 활동에 금액 안 적은 주유가 `0원`.** 합계용 `totalCostOrZero()` 를 한 건 표시에 썼다.
        B-54-1 이 "0원이면 잘못된 것" 이라 정해 둔 것과 정반대였다. `cost` 를 null 가능으로, 화면은 `— 원`.
      → **로그인·가입·`/me` 응답의 숫자 사용자 id.** 차량·기록 id 를 숨긴 이유(가입 순서가 드러남)가
        그대로 적용되는데 남아 있었다. 화면이 쓰는 곳이 0곳이라 공개 id 없이 **빼기만** 했다.
      → **연비 카드가 틀린 이유를 말했다.** 주유량만 비운 두 번째 기록이 있을 때 500km 를 달렸는데
        "주행거리가 늘어난 기록이 없어" 라고 했다. 이유를 단정하지 않는 문구로.
      → **스키마 점검기가 유니크 제약을 못 봤다.** nullable 만 대조해서, 이 저장소가 처음 밟은 함정
        (옛 유니크가 남음)과 공개 id 때 밟을 뻔한 함정(유니크 생성이 조용히 실패)을 둘 다 통과시켰다.
        이름으로 양방향 대조한다. 규칙 6(제약에 이름을 붙인다)이 이걸 가능하게 했다 — 이름이 없으면
        Hibernate 해시 이름이라 "엔티티에 없음" 으로 잘못 잡힌다. 운영 DB 의 유니크 7개는 이름까지 일치.
      → **프로필 저장 뒤 입력칸이 옛 값.** 어제 넣은 서버 쪽 공백 정리가 드러낸 것 — 저장한 값과 칸의
        값이 달라 다음 저장이 "바뀐 것 있음" 으로 같은 요청을 또 보냈다. 응답 값으로 칸을 맞춘다.
      → **가입은 됐는데 자동 로그인이 실패하면 가입 실패처럼 보였다.** 다시 누르면 409. 실패를 둘로
        나눠, 로그인만 실패하면 로그인 화면으로 보내며 "가입했습니다" 를 띄운다(어제 만든 notice 통로).
      → **숫자 연출이 도중에 지금 보이는 값으로 멈추면 `running` 이 true 로 남았다.** 테스트로 먼저
        재현하고(expected true to be false) 고쳤다. 멈춘 숫자에 tabular-nums 가 붙어 폭이 달라지던 자리.
      → **12개월 비용이 전부 0이면 차트 축이 `1 · 0.5 · 0`.** 선은 두고 0 만 적는다.
      → 의심했으나 아니었던 것: 72바이트 넘는 비밀번호로 로그인·변경·탈퇴 → 500 일 줄 알았는데
        BCrypt 가 예외 없이 불일치로 답해 전부 401 이었다(실제 요청으로 확인).
      → 들여쓰기 두 곳(SignUpPage·ForgotPasswordPage). 테스트 백엔드 261 → 264, 프론트 44 → 45.

- [x] 비밀번호 재설정 뒤 로그인 화면이 아무 말도 안 하던 것 (2026-09-26)
      → 성공하면 `/login` 으로 보내기만 해서, 바뀐 건지 몰라 재설정을 한 번 더 시도하게 됐다.
        안내 한 줄을 라우터 `state` 로 넘긴다 — 로그인 화면이 `from`(원래 가려던 곳)을 받는 통로와 같다.
        쿼리(`?reset=1`)로 넘기면 주소에 남아 새로고침할 때마다 다시 뜬다.
      → 로그인에 실패하면 안내 대신 실패만 보인다. 둘이 같이 있으면 무엇이 지금 일인지 흐려진다.
      → 같은 파일의 어긋난 들여쓰기(토큰 없는 경우의 카드)도 맞췄다.

- [x] 세 번째 전체 점검에서 나온 5건 (2026-09-26)
      → **번호판 앞뒤 공백.** DB(unicode_ci)는 뒤 공백을 무시하고 비교하는데(`'12가3456' = '12가3456 '` → 1)
        자바 `equals` 는 구분하고, 서버는 공백을 어디서도 자르지 않았다. 테스트 서버에서 셋 다 재현했다:
        `"7가2925 "` 을 공백만 지워 수정 → **자기 자신과 중복 409** · 앞 공백 번호판 → 같은 번호판 두 대 ·
        공백 없는 번호판의 백업 가져오기 → 409 로 **가져오기 전체 실패**. 폰 자동완성이 끝에 공백을 붙이는
        일이 흔해 실제로 겪을 수 있는 자리다.
        서비스가 번호판·제조사·모델명·닉네임을 `strip()` 하고, "같은 번호판인가" 를 자바에서 보는 두 곳
        (차량 수정·가져오기)은 DB 와 같은 기준(공백·대소문자 무시)으로 본다. 운영 DB 는 `CHAR_LENGTH` 로
        세어 공백 섞인 값이 0건임을 확인했다 — 정리할 데이터는 없었다.
      → **예상 못 한 500 에 문구가 없었다.** 규칙 11 은 "500 도 우리 모양으로" 인데 실제로는 DB 제약 위반만
        그랬다. `ResponseEntityExceptionHandler` 를 이어받아 405·415·404 는 부모가 상태 코드를 지키게 하고,
        나머지만 500 + 우리 문구로 받는다. 부모가 이미 맡은 두 예외는 덮어쓰기로 옮겼다(둘이면 기동 실패).
      → **주유량이 원래 비어 있던 기록은 메모만 고쳐도 매번 확인 창이 떴다.** 수정일 때는 이번에 새로
        비운 경우만 묻는다 — 늘 뜨는 창은 아무도 안 읽는다.
      → **로그아웃 없이 다른 계정으로 로그인하면 세션이 두 사람 목록에 남았다.** 로그인이 세션을 id 만
        바꿔 다시 쓰기 때문이다. 옛 주인이 비밀번호를 바꾸면 지금 주인이 로그아웃됐다. 등록할 때 다른
        사용자 목록에서 먼저 뺀다.
      → **문서:** 공개 id 로 바꾸며 사라진 `findByIdAndVehicleId` 가 CLAUDE.md 3곳·README 1곳에 남아
        있었다. 예외 처리기 설명의 "유니크가 아니면 다시 던져 500" 도 9/23 이전 설명이라 함께 고쳤다.
      → 테스트 백엔드 255 → 261.

- [x] 로그인 응답 시간으로 가입 여부가 드러나던 것 (2026-09-26)
      → 실패 문구는 통일해 뒀는데(규칙 15) **시간이 대신 말하고 있었다.** 이메일이 없으면 `orElseThrow` 에서
        바로 던져 BCrypt 비교를 건너뛰었다. 테스트로 먼저 재서 드러냈다: 없는 이메일 0.11ms,
        비밀번호 틀림 59.6ms — 약 550배. 네트워크 너머에서도 확실히 구분되는 차이다.
      → 없는 계정이면 미리 만들어 둔 `dummyHash` 와 비교를 한 번 돌리고, 결과와 상관없이 같은 문구로 실패한다.
        `dummyHash` 는 상수가 아니라 **같은 인코더로 기동할 때 만든다** — 강도를 바꾸면 같이 따라온다.
      → 실제 서버(HTTP)로도 쟀다: 가입된 이메일 73~85ms / 없는 이메일 77ms. 겹친다.
      → 테스트는 평균이 아니라 **다섯 번 중 가장 빠른 값**을 비교한다. 평균은 GC·JIT 한 번에 끌려가
        흔들리고, 기준선도 "절반" 으로 넉넉하게 뒀다 — 고치기 전에는 550배라 절반 선이 전혀 애매하지 않다.
      → 남은 것: 가입(409)은 존재를 알려주는 것이 의도이고 IP 로 속도를 제한한다(규칙 15). 재설정 요청은
        9/25 에 메일을 비동기로 돌려 큰 차이를 없앴다. 토큰 저장 몇 ms 는 남아 있다.
      → 테스트 백엔드 254 → 255.

- [x] 정비·주유 기록도 공개 id 로 (2026-09-26)
      → 차량만 바꾼 직후 사용자 요청으로 넓혔다. 기록 id 는 주소창에는 안 뜨지만 수정·삭제 요청 경로와
        응답에 실려, 개발자 도구를 열면 "이 서비스에 기록이 몇 건" 이 그대로 보였다.
      → 차량과 같은 모양이다: 생성자에서 `PublicId.generate()`, `updatable = false`, 이름 붙인 유니크
        (`uk_maintenance_records_public_id` · `uk_fuel_records_public_id`). 조회는
        `findByPublicIdAndVehicleId` — **차량 조건을 그대로 건다.** 공개 id 를 알아도 다른 차량 경로로는
        못 건드려야 하는 것은 숫자 id 때와 같다.
      → **정렬은 여전히 숫자 id 로 한다.** 홈 최근 활동이 같은 날짜를 id 내림차순으로 가르는데, 공개 id 는
        무작위라 순서를 말하지 못한다. 서버 안에서 (활동, 숫자 id) 짝으로 정렬을 끝낸 뒤 응답에만 공개 id 를
        싣는다. 연비의 직전 기록 찾기(`findPrevious`)도 숫자 id 로 동점을 가른다 — 둘 다 응답 밖의 일이다.
      → 연비 요약의 `latestRecordId`·`resetPointId` 도 공개 id 다. 화면이 그 값으로 PATCH 를 보내기 때문이다.
      → 경로 변수 타입 불일치(400) 테스트를 **세 번째로** 옮겼다. 차량에 이어 기록 id 도 문자열이 되어,
        타입 있는 경로 변수는 정비 종류(enum)만 남았다.
      → 운영 DB 는 SQL 을 먼저 — README 의 블록을 세 테이블로 합쳤다(차량용도 아직 실행 전이었다).
        테스트 스키마 임시 테이블 20행으로 겹침 없음을 확인했다.
      → 테스트 백엔드 252 → 254.

- [x] 차량 주소를 공개 id 로 — `/vehicles/1` → `/vehicles/k3Xq9mTa2LpZ` (2026-09-26)
      → **남의 차를 못 여는 것과 번호가 아무것도 말하지 않는 것은 다른 일이다.** 404 통일(규칙 11)로
        앞의 것은 이미 막혀 있었지만, 1,2,3… 은 그 자체로 "이 서비스에 차가 몇 대" 와 등록 순서를
        말한다. 사용자가 "내 차가 몇 번째인지 몰랐으면" 해서 시작했다.
      → 차량마다 12자 무작위 `public_id`(SecureRandom, 약 71비트)를 두고 URL·API 에는 그것만 낸다.
        내부 PK·외래키·조인은 숫자 그대로. 서비스는 공개 id 로 소유 차량을 찾은 뒤 `getId()` 로 조회한다.
      → 버린 대안: **PK 를 UUID 로**(외래키 네 곳 교체 + InnoDB 가 무작위 PK 를 중간에 끼워 넣음) ·
        **Hashids/Sqids**(되돌릴 수 있는 변환이라 숨기는 게 아님) · **번호판을 URL 에**(개인정보가
        기록·로그에 남고, 번호판을 고치면 주소가 깨짐) · **사용자별 순번**(몇 번째 차인지는 그대로 보임).
      → **운영 DB 는 SQL 을 먼저 실행해야 한다**(README). `ddl-auto` 가 NOT NULL 컬럼을 더하면 기존 행이
        `''` 로 채워지고 유니크 제약이 조용히 실패한다 — 이 저장소가 세 번째로 만난 같은 종류의 함정이다.
        SQL 은 테스트 스키마의 임시 테이블 7행으로 먼저 검증했다.
      → 경로 변수 타입 불일치(400) 테스트는 차량 id 가 문자열이 되어 무엇이든 받게 되면서
        `recordId` 쪽으로 옮겼다. 핸들러는 그대로다.
      → 정비·주유 기록 id 는 숫자로 남겼다. 주소창에 안 뜨고 API 경로에서만 보인다.
      → 테스트 백엔드 249 → 252.

- [x] 전체 점검에서 나온 7건 (2026-09-25)
      → **CSRF 가 막은 403 을 화면이 "서버에 연결하지 못했습니다" 로 읽었다.** CORS 헤더를
        `addCorsMappings` 가 붙이는데 이건 필터 **뒤**(MVC)에서 동작해서, 필터가 직접 쓴
        응답에는 헤더가 없었다. 브라우저는 그 응답을 못 읽고 fetch 를 네트워크 오류로 끝낸다.
        curl 로 확인: 토큰 없는 POST 는 403 + `Access-Control-Allow-Origin` 없음.
        CORS 를 `FilterRegistrationBean<CorsFilter>` 로 옮겨 맨 앞(`HIGHEST_PRECEDENCE`)에 세웠다.
      → **비밀번호를 바꾸거나 재설정해도 다른 기기의 세션이 살아 있었다.** 세션을 14일로 늘린
        날이라 커졌다 — 훔친 세션이 재설정 뒤에도 2주 산다. 탈퇴한 계정의 다른 기기 세션은
        매 요청 500 이었다. `LoginSessionRegistry` 가 사용자별 세션을 들고 있다가
        변경은 지금 세션만 남기고, 재설정·탈퇴는 전부 끊는다.
        **DB 세션 버전 방식을 안 쓴 이유**: 요청마다 사용자 조회가 하나 늘고, `WebConfig` 가
        리포지토리를 알게 되어 컨트롤러 테스트 9개 전부에 목이 필요해진다. 세션 자체가 톰캣
        메모리에 있으니 목록도 메모리면 수명이 같다(재시작하면 둘 다 사라진다).
      → **재설정 응답 시간이 가입 여부를 알려줬다.** 메일을 요청 스레드에서 동기로 보내서,
        메일 설정이 없는 상태에서도 가입된 주소 12~57ms / 없는 주소 5ms 로 갈렸다. 실제 SMTP 면
        초 단위다. 코드 주석은 "응답 시간으로 드러나지 않는다" 고 말하고 있었다.
        겸사겸사 **커밋 전에 메일이 나가던 것**도 고쳤다 — 롤백되면 저장 안 된 토큰이 메일로 갔다.
        `PasswordResetMailer` 가 커밋 뒤(`TransactionSynchronization.afterCommit`) 스프링 기본
        실행기로 넘긴다. `@TransactionalEventListener` + `@Async` 는 이벤트 클래스와 `@EnableAsync`
        설정이 새로 필요한데 발송하는 곳이 여기 하나라 값을 못 했다.
      → **정비 종류 필터가 버튼보다 컸다.** 문자열로 이어 붙인 `controlClassName + ' h-8 w-auto …'`
        에서 `h-11`·`w-full`·`md:text-[0.9375rem]` 이 CSS 배치 순서로 이겼다(빌드 CSS 에서 확인).
        `cn()` 으로 하나만 남기고, 크기는 임의 값으로 적었다 — `text-caption` 을 넣으면 `cn` 이
        색으로 읽어 `text-strong` 을 지운다. **모바일에서는 16px 를 유지한다** — 전에는 모바일에서
        13px 이 이겨 iOS 확대까지 걸려 있었다.
      → **프로필 수정 중 세션이 끊겨도 로그인 상태로 남았다.** 401 전역 처리 예외 목록이
        경로만 봐서 `GET /me`(정상 401)와 `PATCH /me`(세션 만료)를 못 갈랐다. 메서드까지 본다.
      → **가입만 빈 전화번호를 `''` 로 저장했다**(규칙 14-1). 화면이 걸러 줘서 안 보였을 뿐이다.
      → **같은 주행거리 두 건이 페이지 경계에 걸리면 한 구간이 두 번 보였다.** 직전 기록을
        주행거리만으로 찾아서 둘 다 더 앞 기록을 짝으로 잡았다. (주행거리, id) 순서로 찾는다 —
        목록 정렬·연비 계산과 같은 기준. 괄호 친 OR 가 필요해 처음으로 `@Query` 를 썼다.
      → 덤: oxlint 경고 0건(`buttonVariants` 를 내보내지 않는다 — 쓰는 곳이 없었다),
        CLAUDE.md 의 낡은 설명 셋(중복된 내보내기 항목·homeStats·NextServiceCard)을 코드에 맞췄다.
      → 테스트 백엔드 238 → 249, 프론트 42 → 44.

- [x] 마무리 4건 — 주기 폼 기본값 · 지남 수 null · 같은 비밀번호 · 타입 스케일 (2026-09-26)
      → **주기 폼이 기본값을 미리 채워서, 그대로 저장하면 조용히 '변경됨' 이 됐다.**
        초기값을 `result.intervalKm`(실제 적용값)으로 둔 탓이다. 값은 같은데 상태만 달라지고,
        나중에 기본 권장 주기를 손보면 그 차만 옛 값에 묶인 채 아무도 모른다.
        **폼이 스스로 모순되기도 했다** — 도움말은 "비우면 기본값을 씁니다" 인데 열면 채워져 있었다.
        덮어쓴 적이 없으면 빈 칸으로 열고, 지금 값은 placeholder 로 보여 준다.
      → **`overdueServiceCount` 가 "안 셌다" 와 "0건" 을 구분하지 못했다.** 목록에서만 채워지는데
        `int` 라 단건 응답에서 0 이 됐다. 0 은 "지난 게 없다" 로 읽힌다 — 나중에 상세 화면이
        이 값을 쓰면 항상 "없음" 이 뜨고 왜 안 나오는지 한참 찾게 된다.
        이 저장소가 연비에서 이미 내린 판단("구간 미성립이면 0 이 아니라 null")과 같은 규칙이라
        `Integer` 로 바꾸고 단건은 null 을 준다.
      → **비밀번호를 같은 값으로 바꿀 수 있었다.** 3차 점검에서 적어 놓고 그때 고친 여섯 건에
        안 들어갔던 것이다. 안 막으면 "바꿨다" 는 안내가 뜨는데 아무것도 안 바뀐다 —
        비밀번호가 샜다고 생각해 바꾸러 온 사람이 안 바뀐 채로 안심하고 나간다.
        재설정에는 두지 않았다. 그쪽은 옛 비밀번호를 모르는 사람이라 "같습니다" 가 도움이 안 된다.
      → **타입 스케일 22곳을 정리했다.** "크기는 전부 토큰으로" 라고 적어 놓고 `text-xs`·`text-sm`
        이 화면 곳곳에 남아 있었다 — 작은 글씨가 **12·13·14·15px 네 단으로 3px 안에 몰려**
        위계가 사라진 상태였다. 규칙 6 이 "단계 사이를 과감하게 벌린다" 라고 말하는 바로 그 구간이다.
      → 역할로 갈라 둘로 접었다: 보조 문장·단위는 `text-caption`(13px),
        배지와 차트 안의 작은 값은 `text-unit`(11px) — 어제 만든 `지남` 배지와 같은 눈금이 됐다.
        `cn()`·`cva()` 를 거치는 네 파일과 `date-input` 의 `text-base`(iOS 확대 방지)는 예외로
        남기고 그 이유를 규칙에 적었다.
      → 백엔드 테스트 236 → **238개**.

---

- [x] 방금 넣은 7건이 남긴 문제 4건 (2026-09-25)
      → **필터가 걸린 채 저장하면 사라진 것처럼 보였다.** 오늘 넣은 종류 필터가 만든 버그다 —
        '엔진오일' 로 거른 상태에서 브레이크패드를 등록하면 목록이 새로 그려지는데 방금 넣은 것이
        없다. 서버가 필터에 맞는 것만 주기 때문이다. **사용자는 실패한 줄 알고 다시 누르고
        같은 기록이 두 건 생긴다.** 저장한 종류를 폼이 알려 주게 하고, 필터 밖이면 필터를 푼다.
        `setFilter` 가 `load` 를 바꿔 저절로 재조회되므로 `reload()` 는 안 부른다(두 번 나감).
      → **백업에서 차량별 주기가 빠져 있었다.** 내보내기 → 탈퇴 → 가져오기를 하면
        차량·정비·주유는 돌아오는데 합성유 10,000km 설정만 말없이 사라졌다 —
        **어제 고친 문제가 복원 한 번에 되살아나는** 셈이었다. 내보내기에 넣고(쿼리 3 → 4번),
        가져오기가 되살린다.
      → 복원 규칙은 차량 정보와 같다: **이미 설정이 있는 종류는 건드리지 않는다.**
        파일이 옛날 것일 수 있는데 지금 설정을 밀어낼 이유가 없다. 되살린 개수는 응답에 담아
        화면이 말한다. 옛 파일에는 그 필드가 없으므로 접근자가 null 을 빈 목록으로 바꿔 준다.
      → **차량 목록에는 `지남` 이 없었다.** 홈에만 있어서 같은 질문("어느 차에 할 일이 있나")에
        두 화면이 다르게 답했다 — 헤더의 `내 차량` 으로 곧장 들어간 사람은 못 봤다.
      → 그래서 `VehicleService.findMyVehicles` 가 **엔티티가 아니라 DTO 를 돌려준다.**
        지남 수가 엔티티에 없는 계산값이라서다 — `FuelRecordService.findByVehicle` 이 연비 때문에
        같은 선택을 했던 자리와 같다. 쿼리는 1 → 3번이 됐다(홈과 달리 공짜가 아니다).
        계산은 같은 `NextService` 를 쓴다 — 두 화면이 다른 수를 말하면 안 된다.
      → **카드 머리말이 375px 에서 넘칠 수 있었다.** 오늘 종류 select 를 그 줄에 넣었는데
        `flex-wrap` 도 `min-w-0` 도 없었다. 줄여서 맞추지 않고 접는다(규칙 11-1).
        주유 카드도 같은 구조라 함께 고쳤다.
      → 띄워서 확인했다: 주기 10,000km 설정 → 내보낸 JSON 에 `serviceIntervals` 포함 →
        새 계정에 복원 → **복원된 주기 10,000km · 지남=false · 변경됨=true.**

---

- [x] 기능 공백 7건 — 급증 확인 · 차량별 주기 · 연비 추이 · 세션 · 빠른 진입 · 필터 · 가져오기 (2026-09-25)
      → **주행거리가 크게 뛸 때 아무도 안 물었다.** 줄어드는 것은 확인 창으로 막으면서
        늘어나는 쪽은 무방비였는데 **방향이 거꾸로다** — 줄이는 것은 force 로 되돌릴 수 있지만
        올라간 값은 liftOdometerTo 가 최댓값으로 붙잡아 force 정정 말고는 길이 없다.
        기준을 배수와 절대량 둘로 둔 이유: 배수만 보면 1,000 → 5,000 같은 새 차의 정상 입력을
        잡고, 절대량만 보면 4,500 → 45,000(자리수 오타)을 놓친다. 기존 값이 0 이면 묻지 않는다.
        규칙을 `shared/lib/odometer` 로 모으면서 두 폼에 복사돼 있던 `looksPast` 도 합쳤다.
      → **권장 주기를 차량마다 바꿀 수 있게 했다.** 어제 만든 `지남` 의 짝이다 —
        엔진오일 기본값이 광유 기준 5,000km 인데 합성유는 10,000~15,000km 라,
        손잡이가 없으면 **`지남` 이 늘 켜진 경고등**이 되고 늘 켜진 경고는 아무도 안 본다.
      → 차량 단위인 이유: 같은 사람이 디젤과 가솔린을 함께 몰 수 있고 주기는 차의 성질이다.
        km·개월을 따로 비울 수 있다 — 합성유는 거리만 늘고 기간(6개월)은 그대로인 게 보통이다.
        둘 다 비면 행을 지운다. 남겨 두면 `customized` 가 참으로 남아 화면이 거짓말한다.
      → PUT 이 뜻에는 맞지만 PATCH 로 갔다. PUT 을 열면 CORS 의 allowedMethods 와 fetch 래퍼에
        메서드를 하나 더 늘려야 하는데, 이 앱에 PUT 이 필요한 자리가 여기뿐이라 값을 못 한다.
      → **연비 추이를 볼 수 있게 했다.** 이 앱이 계산하는 값 둘 중 하나인데 "지금 평균" 하나만
        보여 줬다 — 20번을 기록해도 좋아지는지 나빠지는지 알 수 없었고, **연비가 꾸준히 떨어지는
        것은 그 자체로 정비 신호**다. `summary` 가 이미 전체 기록을 읽고 있어 **추가 쿼리는 0**.
      → 세로축을 0 이 아니라 최솟값의 90% 에서 시작한다. 0 부터 그리면 12~14km/L 같은 실제 차이가
        막대 끝 몇 px 로 뭉개진다 — 여기서 보려는 것은 절대량이 아니라 변화다. 대신 그 사실을 적었다.
        평균에서 뺀 구간(불가능한 값·기록 빠짐)은 여기서도 뺀다. 넣으면 한 점이 축을 혼자 차지한다.
      → **세션이 톰캣 기본 30분이었다.** 쿠키 속성은 "브라우저 기본값에 기대지 않는다" 며 다 적어
        두고 정작 만료만 기본값이었다. 14일로 늘리면서 `timeout` 과 `cookie.max-age` 를 **같이**
        바꿨다 — 서버가 14일을 기억해도 세션 쿠키면 브라우저를 닫는 순간 끝난다.
      → **홈에서 기록까지 네 단계였다.** 차량이 한 대면 그 차로 바로 보낸다. 여러 대면 목록으로 —
        고를 것이 있을 때만 고르게 한다.
      → **정비 이력에 종류 필터를 뒀다.** 네이티브 select 를 쓴 이유: 종류가 15개라 세그먼트
        컨트롤로는 줄이 넘치고, 직접 만든 드롭다운은 키보드·스크린리더를 처음부터 다시 짜야 한다.
        필터가 걸린 채 비면 "아직 등록된 정비 이력이 없습니다" 는 **거짓말**이라 문구를 갈랐다.
      → **가져오기를 만들었다.** 화면이 "탈퇴하면 복구되지 않습니다. 받아 두세요" 라고 하면서
        받은 파일로 할 수 있는 일이 없었다 — **복원할 수 없으면 백업이 아니라 기념품이다.**
      → 규칙 셋: 같은 번호판이면 기록만 붙이고 차량 정보는 안 건드린다(파일이 옛날 것일 수 있다) ·
        **같은 기록은 건너뛴다** · 하나라도 검증에 걸리면 전부 안 들어간다.
        두 번째가 제일 중요하다 — 이 기능에서 가장 나쁜 결과는 "실수로 두 번 눌렀더니 두 배" 다.
        내보낸 JSON 에는 id 가 없어(우리 DB 안에서만 뜻이 있어 뺐다) 종류·날짜·주행거리로 판정한다.
      → 패키지가 `restore` 인 이유는 **import 가 자바 예약어**라서다.
      → 사용자 정보(이메일·닉네임)는 받지 않는다. 가져오기는 내 계정에 기록을 더하는 것이지
        계정을 바꾸는 게 아니다 — 남의 파일을 넣어도 내 계정에 붙는다.
      → 띄워서 확인했다: 내보내기 → 다른 계정에 넣기(차량 1·정비 1·주유 2) → **같은 파일 또 넣기
        (0 추가, 3 건너뜀, 기록은 그대로 3건)** → 잘못된 파일은 어느 필드인지까지 400.
      → 백엔드 테스트 227 → **236개**.

---

- [x] 지난 정비를 표시한다 — 계산만 하고 결론을 말하지 않던 것 (2026-09-25)
      → 점검에서 나온 **기능 공백**이다. `다음 정비 시점` 카드가 날짜와 주행거리를 계산해
        보여주는데, **이미 지난 것과 아직 안 온 것이 완전히 똑같이 보였다.** 저장소 전체에
        `지남/임박/overdue` 를 뜻하는 코드가 한 줄도 없었다(grep 0건).
      → 이게 컸던 이유: README 첫 줄과 랜딩·AuthLayout 이 **"다음은 언제인지"** 를 약속하는데,
        정작 결론을 읽는 사람이 날짜를 오늘과 머릿속으로 비교해야 했다.
      → **판정은 서버가 한다.** 화면이 직접 오늘과 비교하면 차량 상세와 홈이 다른 말을 하게 된다 —
        연비 공식을 하나로 합친 것(2026-09-17)과 같은 이유다. `NextService` 를
        `maintenance/domain/calculation/` 에 뒀다. `fuel/domain/calculation` 과 같은 자리다.
      → **km·개월 중 하나라도 넘으면 지남.** 권장 주기가 "먼저 오는 것"이기 때문이다 —
        차를 거의 안 타면 주행거리는 한참 남아도 오일은 굳는다. 딱 그 값·그 날도 지난 것으로 본다.
      → **지난 것을 목록 맨 위로** 올린다. "화면 순서를 서버가 정한다"는 기존 결정은 그대로
        지키면서(안정 정렬이라 나머지는 enum 선언 순서), 이 목록의 용도에 맞췄다.
      → **색을 늘리지 않았다.** 빨강은 "실패"를 나르는 기능색이고 `확인 필요`(입력 오류)가 이미
        쓰고 있다. 정비 시기가 지난 것은 잘못이 아니라 **할 일**이라, 모노톤 시스템의 방식대로
        **대비를 올렸다** — 테두리 + `text-strong`. 노랑·주황을 새로 만들면 규칙 3 이 무너진다.
        디자인 규칙 3 에 이 판단을 적어 뒀다.
      → **홈 `차량별` 에도 `정비 N건 지남` 을 붙였다. 추가 쿼리는 0이다** — `GarageSummaryService`
        가 이미 그 사용자의 정비 이력을 전부 읽고 있어서, 같은 `NextService` 를 돌리기만 하면 됐다.
        차량 상세까지 안 들어가도 어느 차에 할 일이 있는지 보인다.
      → 계산을 도메인으로 뽑으면서 `MaintenanceRecordService` 의 종류별 최신 고르기·주기 더하기가
        통째로 빠졌다. 서비스는 이제 "누구 차인지 확인하고 넘긴다" 뿐이다.
      → **임박(곧 해야 함)은 안 만들었다.** 임계값이 자의적이 되고, 지금 문제는 "지났는지도
        모른다" 쪽이었다. 필요해지면 그때 기준을 정한다.
      → 백엔드 테스트 219 → **227개**(`NextServiceTest` 8개 신설 — 경계값·OTHER·정렬·최신 하나).

---

- [x] 두 번째 전체 점검에서 나온 6건 (2026-09-24)
      → **내보내기가 간헐적으로 빈 파일을 줄 수 있었다.** `link.click()` 바로 다음 줄에서
        `URL.revokeObjectURL` 을 불렀다. click() 은 다운로드를 시작만 시키고 브라우저가 Blob 을
        읽는 건 그다음이라, 같은 틱에 해제하면 취소되거나 0바이트가 떨어진다.
        **이 버튼은 탈퇴 직전에 기록을 챙기라고 둔 것**이라 조용히 실패하면 백업을 받은 줄 알고
        계정을 지운다. `setTimeout(…, 0)` 한 줄.
      → **로그인·가입 화면의 제목 순서가 뒤집혀 있었다.** `AuthLayout` 왼쪽 태그라인이 `h2`
        인데 DOM 에서 `Outlet` 보다 앞서 있어, 스크린리더 목차가 `h2 → h1` 이 됐다.
        게다가 `hidden lg:flex` 라 **화면 폭에 따라 목차가 달라졌다.**
        그 문장은 아무 구역도 이끌지 않으므로 `p` 가 맞다 — `CardTitle` 을 `h2` 로 올린
        근거("카드 제목은 곧 구역 제목")의 정확히 반대 경우다. 규칙 9-1 로 적어 뒀다.
      → **타입 스케일 규칙이 화면 6곳에서 안 지켜지고 있었다.** 문서는 "크기는 전부 토큰으로,
        임의 값은 cn() 을 거치는 네 파일뿐" 이라고 적혀 있었는데 사실이 아니었다.
        **반복되는 값만 토큰으로 올렸다**: `text-headline`(랜딩 h2 ×2) ·
        `text-unit`(11px ×3) · `text-axis`(10px ×4).
      → 나머지(랜딩 h1, 태그라인, 통계 타일)는 임의 값으로 뒀고 **규칙에 기준을 명시했다** —
        쓰는 곳이 하나인 척도를 토큰으로 올리면 척도가 아니라 별명이 된다.
        토큰을 늘렸으므로 `cn-usage.test.ts` 의 `TYPE_SCALE` 목록에도 더했다.
      → **비밀번호 `maxLength={72}` 가 글자 수를 센다.** 실제 제한은 72바이트라 한글이면
        브라우저가 216바이트까지 통과시키고 서버가 400 을 준다. `maxLength` 로는 바이트를 못
        세므로 **도움말이 실시간으로 바이트를 세게** 했다 — 넘치면 현재 바이트 수까지 말한다.
        속성은 거친 천장으로 남겨 두고 왜 그런지 주석을 달았다.
      → **만료된 재설정 토큰이 쌓이고 있었다.** 재발급·탈퇴 때만 지워져서, 요청만 하고 안 쓴
        토큰은 영원히 남았다. 스케줄러를 두지 않고 `request()` 가 부른다 —
        **토큰이 쌓이는 유일한 경로가 거기라 쌓이는 만큼 치워진다.** 가입 여부를 확인하기
        *전에* 부르는 것도 의도다(없는 주소와 하는 일이 같아야 응답 시간이 안 갈린다).
      → **테스트가 고정된 시간대를 안 탔다.** 시간대를 `main()` 에서 고정했는데 테스트는
        `main()` 을 거치지 않아, `@PastOrPresent` 를 쓰는 테스트가 CI(UTC)와 로컬(KST)에서
        다른 조건으로 돌고 있었다. `build.gradle` 의 `systemProperty` 한 줄로 맞췄다.

---

- [x] 점검에서 남아 있던 자잘한 결함 7건 (2026-09-23)
      → **정렬이 연관 엔티티를 타고 들어갔다.** `?sort=owner.password` 가 200 이었다 —
        값이 응답에 실리지는 않지만 암묵적 조인이 생기고 의도한 적 없는 표면이 열린다.
        **블랙리스트가 아니라 화이트리스트로 막았다**: 엔티티에 필드를 하나 더하면 그 필드가
        자동으로 정렬 대상이 되므로, 막을 것을 세는 쪽은 언제나 뒤처진다.
        400 을 우리가 직접 판단하는 첫 자리라 `InvalidRequestException` 을 새로 뒀다(규칙 12).
      → **메모·정비 설명을 비우면 빈 문자열로 저장됐다.** `phone` 은 null 로 정규화하는데
        이 둘만 안 했다. "없음" 이 두 모양이면 **내보낸 JSON 에도 그 차이가 그대로 나간다.**
        자르는 자리는 서비스다 — DTO 접근자에서 자르면 부분 수정에서 "안 보냄"과 "지움"이 같아진다.
      → **앱 시간대를 Asia/Seoul 로 고정했다.** `@PastOrPresent` 와 `LocalDate.now()` 가 JVM
        기본 시간대를 따라서, UTC 서버면 한국 사용자가 고른 "오늘" 이 아직 미래라 **매일 오전
        9시까지 400** 이 난다. 홈 요약의 월별 12칸도 같은 이유로 하루씩 밀린다.
        `SpringApplication.run` 보다 **먼저** 부른다 — 커넥션 풀과 Hibernate 가 뜰 때 한 번
        읽어 가므로 그 뒤엔 늦다. 프런트가 이미 `todayString()` 으로 로컬 기준을 쓰고 있어
        여기를 고정해야 양쪽이 맞는다.
      → **useCountUp 이 연출 도중 값이 또 바뀌면 튀었다.** cleanup 이 `fromRef` 에 옛 목표를
        넣어서, 화면에 보이던 중간값이 아니라 옛 목표에서 새 연출이 출발했다.
        지금 찍혀 있는 값을 따로 들고 있다가 거기서 이어간다.
      → **날짜 휠이 20년 전까지만 만들었다.** 그보다 오래된 기록을 열면 값이 목록에 없어
        `indexOf` 가 -1 이 되고 년 칸이 자리를 못 잡았다. 서버는 `@PastOrPresent` 라 얼마나
        오래된 날짜든 받는데 화면만 못 열면 안 된다 — 지금 값이 더 오래됐으면 그 해까지 늘린다.
      → **ProtectedRoute 가 pathname 만 넘겼다.** 로그인 뒤 복귀할 때 `?page=2` 같은 것이
        사라졌다. search·hash 까지 함께 넘긴다.
      → **문서 드리프트 하나를 치웠다.** `CLAUDE.md` 가 `MaintenanceRecordRepository` 에
        `findTopByVehicleIdAndTypeOrderByServiceDateDescIdDesc` 가 있다고 적고 있었는데,
        9/16 에 `/next-services` 로 합치면서 사라진 메서드다.
      → 백엔드 216 → **219개**.

---

- [x] 입력 상한과 날짜 휠 보정 (2026-09-23)
      → **B-4. 주행거리·금액에 상한이 없었다.** `{"odometer": 2000000000}` 이 200 으로 통과했다.
        **상한의 목적은 "말이 되는 값인가" 가 아니라 되돌릴 수 없게 망가지는 것을 막는 것**이다 —
        `liftOdometerTo` 가 "지금까지 기록된 최댓값" 을 잡아 두기 때문에, 한 번 20억이 들어가면
        그 뒤 모든 정비·주유 폼이 *"차량에 기록된 2,000,000,000km 보다 작습니다"* 를 띄운다.
        되돌리는 길은 force 정정 하나뿐이다.
      → 200만 km / 1억 원. 기네스 최고 기록이 약 500만 km 지만 그건 한 대의 이야기고,
        200만이면 정상적인 입력은 전부 통과하면서 자리수 실수는 걸린다.
      → 숫자를 애노테이션에 직접 적지 않고 `InputLimits` 에 모았다 — 주행거리 4곳·금액 4곳에
        흩어지면 나중에 한 곳만 고친다. 프런트에도 같은 숫자를 두어 **브라우저가 먼저 막는다**
        (연식의 `@Min(1900)` + `min` 과 같은 이중 방어). 의도한 중복이라 양쪽에 적어 뒀다.
      → **B-3. 날짜 드럼 휠이 올해 남은 달을 열어 두고 있었다.** 데스크톱은
        `<input type="date">` 의 `max` 로 막는데 휠은 년만 올해까지고 월·일은 12/31 까지 자유라,
        오늘이 9/23 이면 석 달이 열려 있었다. 고르면 저장할 때 서버가 400.
        그 자리 주석은 *"기기마다 다르게 동작하지 않게"* 라고 적혀 있었다 — 년 단위로만 참이었다.
      → 규칙을 `date-parts` 에 뒀다(`lastSelectableMonth`/`lastSelectableDay`). 칸 목록과
        `join` 의 보정이 **같은 함수를 본다** — 두 벌이면 보이는 칸과 저장되는 값이 갈린다.
      → `join` 이 **년 → 월 → 일 순서로** 자른다. 굴린 칸의 뜻을 최대한 살리려는 것이다:
        2020-12-25 에서 년만 올해로 굴리면 12월이 미래라 9월로, 25일도 미래라 23일로 내려온다.
        1/31 에서 2월로 옮기면 28일이 되는 것과 같은 성격의 보정이다.
      → ⚠️ **기존 `join` 테스트들이 그 순간 시각 의존적이 됐다.** `join(2026, 4, 31)` 은
        2026년 3월에 돌리면 3월로 잘려 깨진다. 그 describe 블록에도 시각을 고정했다 —
        고치면서 만든 시한폭탄을 같이 치운 것이다.
      → 백엔드 214 → **216개**, 프런트 39 → **42개**.

---

- [x] 점검 결함 4건 — 거짓 라벨 · 400 본문 · int 오버플로 · 스키마 어긋남 감지 (2026-09-23)
      → **A-1. 주유량 없는 기록이 "기준 기록"이라고 거짓말했다.** 바로 앞 작업에서 내가 만든
        회귀다 — 연비가 없는 이유가 둘에서 셋으로 늘었는데 화면은 둘만 알고 있었다.
        새 조건을 만들지 않고 `distance` 로 가려냈다: **기준점이거나 직전 기록이 없으면 서버가
        distance 도 null 로 주므로**, distance 가 있는데 연비가 없다는 것은 "달린 거리는 아는데
        얼마나 넣었는지 모른다" 하나뿐이다.
      → **B-2. 읽을 수 없는 요청 본문에 핸들러가 없었다.** 깨진 JSON·없는 enum·날짜 형식 오류가
        스프링 기본 응답(timestamp/status/error/path)으로 나갔고, `message` 가 없어 화면이
        "요청에 실패했습니다 (HTTP 400)" 로 떨어졌다. 덤으로 내부 경로가 응답에 실렸다.
        Jackson 의 원인 메시지를 그대로 내보내지는 않는다 — 패키지·클래스 이름이 그대로 담긴다.
        대신 **어느 필드인지만** 뽑아 준다. 고치는 데 필요한 것은 그것뿐이다.
      → 같은 자리에서 `DataIntegrityViolationException` 의 재던짐도 바꿨다.
        **"상태 코드를 낮추지 않는다" 와 "본문을 주지 않는다" 는 다른 얘기다** —
        500 은 500 으로 두되 ErrorResponse 는 돌려준다. 예외를 다시 안 던지므로 스택은 직접 찍는다.
        사용자가 주유량을 비우고 저장했을 때 화면에 "HTTP 500" 만 떴던 게 이 자리다.
      → **B-1. 차량 연비 요약의 총 유류비가 int 였다.** 40억 원이 -294,967,296 으로 찍혔고,
        같은 데이터에 홈 요약(long)은 4,000,000,000 을 줬다 — **같은 값을 두 화면이 다르게
        말하는 상태**였다. int → long.
      → **C-1. ddl-auto 함정을 네 번째로 밟지 않게 `SchemaDriftChecker` 를 뒀다.**
        기동할 때 엔티티의 nullable 과 실제 컬럼을 대조하고, 어긋나면 고칠 ALTER 까지 찍는다.
      → ⚠️ **처음에 권했던 `ddl-auto: validate` 는 틀렸다.** 실험해 보니 Hibernate 의 스키마
        검증은 테이블·컬럼의 **존재와 타입만** 보고 nullability 는 아예 보지 않는다 —
        일부러 어긋나게 만든 스키마에 validate 를 걸어도 앱이 그냥 떴다. 그래서 직접 대조한다.
      → @Column·@JoinColumn 이 붙은 필드만 본다. 애노테이션이 없으면 기본값을 추측해야 하는데
        기본형(int)에 Hibernate 가 NOT NULL 을 붙이는 등 예외가 많아 **없는 어긋남을 보고하게 된다.**
      → 막지 않고 경고만 한다 — 레거시 컬럼 하나로 앱이 안 뜨면 그게 더 큰 방해다.
        대신 SQL 로그에 묻히지 않게 한 덩어리로 찍는다.
      → 검사 로직과 로그를 나눠 뒀다(`findDrifts()`). 로그를 가로채 문자열을 뒤지는 테스트는
        문구를 고칠 때마다 깨진다. **이 장치가 조용히 고장 나면 그때부터 아무것도 못 잡는다.**
      → 백엔드 테스트 207 → **214개**.

---

- [x] 주유량·결제 금액을 선택 입력으로 (2026-09-23)
      → 사용자 요청. `required` 를 떼고, 비운 채 저장하려 하면 무엇을 못 하게 되는지 창으로 알린다.
      → **프런트만 고쳐서는 안 되는 일이었다.** 백엔드가 `@NotNull` 이고 DB 컬럼이 `NOT NULL` 이라,
        입력칸만 열어 두면 저장할 때 400 이 난다. 엔티티·요청 DTO·계산 둘·응답·요약·홈 요약·
        내보내기까지 번졌다.
      → **0 으로 채우지 않았다.** 0 은 "0L 를 0원에 넣었다" 라는 **다른 사실**이고, 연비가
        0 으로 나누기가 되며 유류비 합계가 조용히 틀어진다. 원칙 8("없음이 있는 값만 래퍼 타입")
        그대로 `int totalCost` 를 `Integer` 로 바꿨다.
      → **거리와 연비는 조건이 다르다.** 처음에 둘을 한 조건에 묶었다가 주유량이 없으면
        "500km 를 달렸다" 는 사실까지 같이 사라졌다 — 새로 쓴 테스트가 바로 잡아냈다.
        모르는 것은 "얼마나 넣었나" 뿐이고 계기판은 이어진다.
      → **경고를 모아 한 창으로 띄운다.** 주행거리 미수정 경고가 이미 있어서, 조건마다 창을
        띄우면 두 개가 연달아 뜬다. **두 번째 창은 사람이 읽지 않고 누른다.**
        대신 비운 칸마다 무엇을 잃는지는 다르므로 줄을 나눠 적는다 —
        주유량은 연비, 결제 금액은 유류비 합계와 단가다.
      → **수정에서 '유지'와 '비움'을 가르는 데 한 번 막혔다.** `Optional<BigDecimal>` 로 감싸면
        될 줄 알았는데, **Jackson 은 키가 아예 없을 때도 `Optional.empty()` 를 채운다**(테스트로
        확인). 그대로 뒀다면 **메모만 고치는 요청이 주유량을 지웠을 것이다.**
        결국 `clearLiters`·`clearTotalCost` 불리언을 따로 뒀다 — 덜 우아해 보여도 헷갈릴 여지가 없다.
      → 목록은 `0.00 L` 이 아니라 **`— L`** 로 적는다. 0 을 찍으면 적은 값처럼 보인다.
      → **운영 DB 에 ALTER 한 번이 필요하다**(README 에). `ddl-auto: update` 가 NOT NULL 을
        못 지우는 함정을 이 저장소가 밟는 **세 번째**다.
      → 백엔드 테스트 202 → **207개**.

---

- [x] 기록을 지우면 다음 구간 연비가 뛰던 문제 — 빠진 구간을 평균에서 제외 (2026-09-23)
      → 사용자가 "주유 기록을 삭제하면 연비들이 하나씩 차례대로 너무 높게 나온다"고 알려 왔다.
        **산술 버그가 아니라 모델의 구멍이었다.** 구간 연비는
        `(이번 odometer − 직전 odometer) ÷ 이번 주유량` 인데, 기록을 지우면 **거리는 앞 기록까지
        늘어나는데 지워진 기록의 주유량은 사라진다.** 한 번 지우면 한 행만 튀지만,
        **튄 행이 이상해 보여 그것도 지우면 그 다음 행이 또 튄다** — "차례대로"의 정체가 이것이다.
      → **깜빡하고 안 적은 것과 지운 것은 물리적으로 같은 사건이다.** 그래서 새 개념을 만들지
        않았다 — 이미 `FuelAnomaly.longSegmentCount` 가 그 사건을 부르는 이름이었다.
      → **진짜 문제는 탐지와 산술이 연결돼 있지 않았다는 것이다.** `isImpossible` 은 절대
        임계값(50 초과)이라 14 km/L 차가 28 이 되어도 안 걸리고, `longSegmentCount` 는 요약 카드에
        문장 한 줄을 띄울 뿐 **평균 계산에 전혀 관여하지 않았다.** 감지해 놓고 쓰지 않았다.
      → `FuelAnomaly` 를 "몇 개냐"에서 **"이 구간이 수상한가"** 로 바꿨다(`Baseline`).
        기준을 한 번 만들어 구간마다 물어보는 모양이라 **목록과 평균이 같은 기준을 쓴다** —
        공식이 두 벌이면 화면마다 다른 말을 한다는 2026-09-17 의 교훈과 같은 자리다.
      → **거리와 연비가 둘 다 평소의 1.8배를 넘을 때만 잡는다.** 거리만 보던 옛 규칙은
        **장거리 여행을 오탐했다** — 멀리 가면 그만큼 넣으므로 거리는 길어도 연비는 평소와 같다.
        기록이 빠진 구간만 거리가 늘고 주유량이 그대로라 연비까지 함께 뛴다. 테스트로 고정했다.
      → **빼는 이유가 둘이라 개수도 둘로 나눴다**(`excludedSegments` / `missingSegments`).
        합치면 안내가 하나가 되는데, **고치는 방법이 다르다** — 앞은 적힌 값을 고치는 일이고
        뒤는 없는 기록을 채우는 일이다.
      → 요약이 빠진 구간 수를 **따로 세지 않고 계산 결과를 그대로 쓴다.** 평균에서 뺀 바로 그
        개수라야 "N곳을 뺐습니다" 가 참이 된다.
      → 목록 행에는 **`기록 빠짐?`** 이 붙는다. **빨강을 쓰지 않았다** — 빨강은 "실패"를 나르는
        기능색인데 이건 잘못이 아니라 빈자리다. 숫자는 지우지 않는다(`확인 필요` 와 같은 원칙).
        **한 행에 표시가 둘 붙지는 않는다** — 무엇을 하라는 건지 흐려지므로 불가능한 값이 먼저다.
      → 목록 조회가 **쿼리 2번 → 3번**이 됐다. 페이지 안에서 중앙값을 내면 **같은 기록이
        1페이지와 2페이지에서 다르게 판정된다.** 기준은 이력 전체라야 한 벌이다.
      → 구간이 3개 미만이면 여전히 판단을 보류한다. 그 구멍은 **삭제 확인 문구**가 메운다 —
        "연비가 다시 계산됩니다"는 틀린 말은 아니지만 **중요한 걸 안 말하고 있었다.**
      → 남은 한계: **만탱크가 아닌 주유**는 그 구간을 *낮게* 만들어 방향이 반대라 이 장치로는
        안 잡힌다. 백로그의 만탱크 플래그가 그쪽 몫이다.
      → 백엔드 테스트 195 → **202개**.
