...
public class GlobalExceptionHandler {
    ...
    /**
     * 중복 검사(existsBy...)와 저장 사이에 다른 요청이 끼어들면, 검사를 통과하고도
     * DB 유니크 제약에서 막힌다. 진짜 방어선은 유니크 제약이고 existsBy 는 친절한
     * 메시지를 만들기 위한 것이라, 두 방어선이 같은 상태 코드(409)를 내도록 맞춘다.
     *
     * 유니크 위반일 때만 409 로 내린다. DataIntegrityViolationException 은 NOT NULL
     * 위반처럼 우리 쪽 버그도 함께 타고 오는데, 그것까지 409 로 감싸면 500 이어야 할 일이
     * 조용히 4xx 로 나가 영영 모르게 된다. 유니크가 아니면 그대로 다시 던져 500 으로 보낸다.
     */
