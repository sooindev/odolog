...
백엔드 API 27개 · 화면 10 라우트가 모두 동작하고 테스트 234개(백엔드 195 · 프론트 39)가 통과한다.
...
- 평소보다 유난히 긴 구간은 "주유 기록이 빠졌을 가능성"으로 알린다. 기준은 평균이 아니라
  **중앙값의 1.8배** — 평균은 잡아내려는 이상값 자체에 끌려 올라간다.
  구간이 3개 미만이면 판단하지 않는다("평소"가 없는데 의심부터 할 수는 없다)
...
13개)이 모두 동작하는 상태다. 백엔드 테스트 **195개**, 프론트엔드 테스트 **39개**가 통과하고,
...
(테스트 **234개**(백엔드 195 · 프론트 39)는 지금 통과 중이다. **남은 것은 사람 눈 확인 하나뿐이다.**)
