...
백엔드 API 27개 · 화면 10 라우트가 모두 동작하고 테스트 280개(백엔드 238 · 프론트 42)가 통과한다.
...
- **주유량과 결제 금액은 비워 둘 수 있다**(2026-09-23). 영수증을 잃었거나 계기판만 적어 두는
  경우가 있어서다. 비운 기록은 **자기 구간의 연비만 빠지고 거리는 그대로 나온다** —
  모르는 것은 "얼마나 넣었나" 뿐이고 계기판은 이어진다. `0` 으로 채우지 않는다:
  0 은 "0L 를 0원에 넣었다" 라는 **다른 사실**이고, 연비가 0 으로 나누기가 된다
...
`정비 N건 지남` 이 붙어서, 차량 상세까지 들어가지 않아도 어느 차에 할 일이 있는지 보인다.
**색을 늘리지 않고 대비로 표현한다** — 빨강은 "실패"를 뜻하는 기능색이고, 정비 시기가 지난 것은
실패가 아니라 할 일이다.
...
### ⚠️ 이미 쓰던 DB 가 있다면 — 주유량·금액 선택 입력 (2026-09-23)

주유량과 결제 금액을 비워서 저장할 수 있게 하면서 두 컬럼을 nullable 로 바꿨다.
**새로 스키마를 만드는 경우에는 할 일이 없다.** 예전부터 쓰던 DB 에만 한 번 실행한다.
...
`ddl-auto: update` 는 제약을 **추가는 해도 지우지는 않는다.** 컬럼이 `NOT NULL` 로 남아 있으면
비운 기록을 저장하는 순간 DB 가 거절해 500 이 난다. 테스트 스키마는 매번 새로 만들어지므로
`./gradlew test` 는 멀쩡히 통과한다 — 이 저장소가 같은 함정을 밟는 **세 번째**다.

**그래서 네 번째를 막는 장치를 넣었다.** 앱이 뜰 때 엔티티의 nullable 과 실제 컬럼을 대조해
어긋나면 고칠 SQL 까지 로그에 찍는다(`SchemaDriftChecker`). 막지는 않고 경고만 한다.

    ╔═════════════════════════════════════════════════════════
    ║ 스키마가 엔티티와 어긋나 있습니다 (1곳)
    ╚═════════════════════════════════════════════════════════
      fuel_records.liters — 엔티티는 nullable, DB 는 NOT NULL
          ALTER TABLE fuel_records MODIFY COLUMN liters decimal(6,2) NULL;
...
`odometer DESC, id DESC` 로 고정한다. 다른 목록 API 는 `sort` 를 받지만 **화이트리스트**다 —
허용 목록 밖은 400 이다. 오타뿐 아니라 `?sort=owner.password` 처럼 **연관 엔티티를 타고
들어가는 정렬**도 여기서 막힌다. 막을 것을 세는 쪽(블랙리스트)은 엔티티에 필드가 늘 때마다
뒤처지기 때문이다.
...
화면도 같은 선을 긋는다 — 데스크톱은 `<input type="date">` 의 `max`, 터치는 드럼 휠의 칸 목록이
오늘에서 끊긴다. 안 맞추면 **기기마다 되는 날짜가 달라진다.**

**2-1. 앱의 시간대는 `Asia/Seoul` 로 고정돼 있다**(`OdoLogApplication.main`). 이 앱의 날짜는
전부 "사람이 쓰는 달력 날짜"인데 `@PastOrPresent` 와 `LocalDate.now()` 는 JVM 기본 시간대를
따르므로, 서버가 UTC 면 한국 사용자가 고른 "오늘"이 아직 미래라 **매일 오전 9시까지 400** 이 난다.

**3-1. 주행거리와 금액에는 상한이 있다**(200만 km / 1억 원). 상한의 목적은 "말이 되는 값인가"가
아니라 **되돌릴 수 없게 망가지는 것을 막는 것**이다 — 차량 주행거리는 기록을 따라 오르기만 하므로
(`liftOdometerTo`), 한 번 크게 잘못 넣으면 그 뒤 모든 폼이 그 값을 기준으로 말하기 시작한다.
...
- 남의 차량은 403 이 아니라 **404** 다. 403 은 "권한 없음"과 동시에 **"존재함"** 을 나르고,
  ...
  상태 코드만 맞추면 메시지가 대신 알려준다. 정비·주유는 `findByPublicIdAndVehicleId` 라 원래 404 하나였다
...
13개)이 모두 동작하는 상태다. 백엔드 테스트 **238개**, 프론트엔드 테스트 **42개**가 통과하고,
...
| 주유량·금액을 선택 입력으로 | 비워서 저장 가능. 대신 무엇을 못 하게 되는지 저장 전에 한 창으로 알린다 | 2026-09-23 |
| 점검 결함 4건 수정 | 주유량 없는 행의 거짓 라벨 · 읽을 수 없는 요청의 400 본문 · 유류비 int 오버플로 · 스키마 어긋남 감지 | 2026-09-23 |
| 입력 상한과 날짜 휠 보정 | 주행거리·금액 상한(200만km/1억원), 드럼 휠이 오늘 이후를 안 만들게 | 2026-09-23 |
...
| 그 7건이 남긴 것 4건 | 필터 걸린 채 저장 · 백업에서 주기 누락 · 차량 목록의 지남 · 좁은 화면 머리말 | 2026-09-25 |
| 마무리 4건 | 주기 폼 기본값 · 지남 수 null · 같은 비밀번호 거부 · 타입 스케일 22곳 정리 | 2026-09-26 |
...
(테스트 **280개**(백엔드 238 · 프론트 42)는 지금 통과 중이다. **남은 것은 사람 눈 확인 하나뿐이다.**)
